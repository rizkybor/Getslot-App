...
<body>
    <div class="relative flex flex-col w-full min-h-screen max-w-[640px] mx-auto bg-white">
            ...
            <div id="Title" class="absolute bottom-0 w-full p-4 pt-0 z-10">
                <div
                    ...
                    <p class="w-fit flex shrink-0 items-center gap-[2px] rounded-full p-[6px_8px] bg-white">
                        <img src="{{ Storage::url($ticket->thumbnail) }}" class="w-4 h-4" alt="star">
                        <span class="font-semibold text-xs leading-[18px]">{{ $ticket->photos->count() }} /
                            {{ $ticket->photos->count() + 1 }}</span>
                    </p>
                </div>
            </div>
        ...
        <main id="details" class="flex flex-col gap-5 px-4 pb-[116px]">
            ...
            <section id="Date" class="flex flex-col gap-[6px]">
                <h2 class="font-bold text-sm leading--[21px]">Activitiy Time</h2>
                <div class="grid gap-4">
                    <div class="flex items-center rounded-3xl p-[14px_16px] gap-4 bg-[#F8F8F9]">
                        <img src="{{ asset('assets/images/icons/timer.svg') }}" class="w-6 h-6" alt="icon">
                        <div class="text-left">
                            <p class="text-sm leading-[21px]">Date</p>
                            <p class="font-bold text-lg leading-[27px]">
                                {{ \Carbon\Carbon::parse($ticket->event_date)->format('j F Y') }}</p>
                        </div>
                    </div>
                </div>
            </section>
            <section id="Travel-with-Juara" class="flex flex-col gap-[6px]">
                <h2 class="font-bold text-sm leading-[21px]">Benefit</h2>
                <div class="grid grid-cols-3 gap-3">
                    @forelse($ticket->benefit as $benefit)
                        {{-- <div class="flex flex-col items-center rounded-3xl p-[14px_16px] gap-3 text-center bg-[#335ff6] border border-gray-600"> --}}
                        <div
                            class="flex flex-col items-center rounded-3xl p-[14px_16px] gap-3 text-center bg-[#335ff6] border border-gray-600">
                            {{-- <img src="{{ asset('assets/images/icons/' . $benefit['icon'] . '.svg') }}" class="w-9 h-9" alt="icon"> --}}
                            <img src="{{ Storage::url($benefit->icon) }}" class="w-9 h-9" alt="icon">

                            <div>
                                <h3 class="font-bold text-sm leading-[21px] text-black">{{ $benefit->name }}</h3>
            ...
            <section id="Management" class="flex flex-col gap-[6px]">
                ...
                <div class="flex items-center justify-between rounded-3xl p-[10px] pr-[14px] bg-[#F8F8F9]">
                    ...
                    <a href="https://wa.me/{{ $ticket->seller->telephone }}" target="_blank">
                        <img src="{{ asset('assets/images/icons/call-orange.svg') }}" class="w-10 h-10" alt="WhatsApp">
                    </a>
                </div>
            </section>
        ...
        <nav id="Bottom-Nav-Book"
            ...
            <div>
                <p class="font-bold text-[22px] leading-[26px]">{{ $ticket->name }}</p>
            </div>
            ...
            @if ($ticket->event_date <= $currentDate)
